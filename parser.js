...
window.normalizeRawData = function(rows) {
    ...
    // Optional 'exclude' param prevents the same column being assigned to two fields
    const findKey = (candidates, exclude = []) => {
        return keys.find(k => {
            const clean = k.trim().toLowerCase();
            if (exclude.includes(k)) return false; // never reuse an already-assigned column
            return candidates.some(c => clean === c.toLowerCase() || clean.includes(c.toLowerCase()));
        }) || "";
    };

    const keySno        = findKey(['s.no', 'sno', 'sl no', 'sr no', 's. no', 'serial', 'sl.no', 's_no', 'index', '#']);
    const keySiteCode   = findKey(['site code', 'siteid', 'site_code', 'site id', 'site_id', 'site no', 'unit code', 'branch code', 'location code', 'site number', 'unit no', 'code', 'site_id']);
    const keyRegion     = findKey(['region', 'zone', 'state', 'circle', 'area', 'reg', 'territory', 'city']);
    // keySiteName explicitly excludes whichever column keySiteCode matched
    ...
    const keySiteName = findKeyExact(
    ...
    const keyCustGroup  = findKey(['customer group', 'customergroup', 'customer_group', 'client group', 'client', 'customer', 'account']);
    const keyStatus     = findKey(['site status', 'sitestatus', 'status', 'active status']);
    const keySupervisor = findKey(['supervisor', 'super visor', 'field supervisor']);

    // Precise matches for management hierarchy
    const keySrManager  = keys.find(k => {
    ...
    const keyStandaloneBilling = keys.find(k => {
        const cl = k.trim().toLowerCase();
    ...
    const keyStandaloneExpense = keys.find(k => {
        const cl = k.trim().toLowerCase();
        if (cl.includes('%') || cl.includes('pct') || cl.includes('share')) return false;
    ...
    const keyStandaloneCons = keys.find(k => {
        const cl = k.trim().toLowerCase();
    ...
    const monthPatterns = [
        { name: 'Jan', keys: ['jan', 'january', '01/', '01-', '1/', '1-'] },
        { name: 'Feb', keys: ['feb', 'february', '02/', '02-', '2/', '2-'] },
        { name: 'Mar', keys: ['mar', 'march', '03/', '03-', '3/', '3-'] },
        { name: 'Apr', keys: ['apr', 'april', '04/', '04-', '4/', '4-'] },
        { name: 'May', keys: ['may', '05/', '05-', '5/', '5-'] },
        { name: 'Jun', keys: ['jun', 'june', '06/', '06-', '6/', '6-'] },
        { name: 'Jul', keys: ['jul', 'july', '07/', '07-', '7/', '7-'] },
        { name: 'Aug', keys: ['aug', 'august', '08/', '08-', '8/', '8-'] }
    ];
    ...
    monthPatterns.forEach(mp => {
        ...
        keys.forEach(k => {
            ...
            if (matchesMonth) {
                if (kLower.includes('bill') || kLower.includes('revenue') || kLower.includes('invoic')) bCol = k;
                else if (kLower.includes('exp') || kLower.includes('manpower') || kLower.includes('salary') || kLower.includes('wage')) eCol = k;
                else if (kLower.includes('consum') || kLower.includes('value') || kLower.includes('material') || kLower.includes('suppli') || kLower.includes('replace') || kLower.includes('repl')) cCol = k;
            }
        });
